...
const MobileNavigation: React.FC<MobileNavigationProps> = ({
  ...
  return (
    <motion.div
      className={`lg:hidden fixed top-16 left-0 right-0 z-40 m-5 rounded-md ${isScrolled
        ? 'bg-white/95'
        : 'bg-white/10'
      ...
      <div className="flex flex-col p-4 space-y-2 max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        {MAIN_NAV_LINKS.map((link) => (
          <motion.div key={link.name} variants={itemVariants}>
            <Link
              href={link.href}
              className={`block py-4 px-4 rounded-2xl text-lg font-medium tracking-wide ${isScrolled
        ...
        {user ? (
          <>
            ...
            <motion.div variants={itemVariants}>
              <Link
                href="/profile"
                className={`flex items-center py-4 px-4 rounded-2xl text-lg font-medium tracking-wide ${isScrolled
                  ? 'text-navy hover:bg-gray-100'
                  : 'text-white hover:bg-white/10'
                  } transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-terracotta focus:ring-offset-2 focus:ring-offset-transparent active:scale-98`}
                onClick={toggleMenu}
            ...
            {/* Sign Out Button */}
            <motion.button
              className={`flex items-center w-full text-left py-4 px-4 rounded-2xl text-lg font-medium tracking-wide ${isScrolled
                ? 'hover:bg-gray-100'
                : 'hover:bg-white/10'
          ...
          <motion.div variants={itemVariants} className="pt-2">
            <Link href="/auth" onClick={toggleMenu}>
              <Button
                className={`w-full text-lg font-medium tracking-wide rounded-2xl ${isScrolled ? 'bg-black text-white' : 'bg-white text-black'
